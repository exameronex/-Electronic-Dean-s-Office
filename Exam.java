...
public class Exam {
    ...
    void opi1(JFrame frame){
        ...
        JButton vod = new JButton("Ввод результатов экзамена");
        vod.setBounds(500,100,400,70);
        vod.addActionListener(new Vod());
        JButton vivod = new JButton("Вывод результатов экзамена");
    ...
    public class Vod implements ActionListener{
        public void actionPerformed(ActionEvent event){vod();}}
    void vod(){
        fr.getContentPane().removeAll();
        ...
        public class Ok implements ActionListener{
            ...
            {  fr.getContentPane().removeAll();
                ...
                String h = null;
                String[] columnName = {"ФИО","1ат", "2ат", "3ат", "общий балл", "балл за экзамен", "оценка", };
                    ...
                    while(rs.next()){ if((Year-Integer.parseInt(rs.getString("YEAR_OF_FORMATION")))<=4){
                        if(8-Month>0){
                            if(s.equals(rs.getString("GROUP_NAME")))
                                h = (rs.getString("id")); }}}p=0;
                    rs = stmt.executeQuery("SELECT * FROM STUDENT");
                    while(rs.next()){ if(h.equals(rs.getString("ID_GROUP"))) {p++;}}
                    matrixA = new String[p][7]; int g=0;
                    rs = stmt.executeQuery("SELECT * FROM STUDENT");
                        ...
                        if(matrixA[g][0].equals(rs.getString("FIO"))) {
                            if(modul.equals(rs.getString("MODUL"))){
                                if(dics.equals(rs.getString("DISC"))){
                                    matrixA[g][1] =  rs.getString("BAL"); g++;
                                    if(g==p){ break;}}}}}
                    g=0;
                    while(rs.next()){
                        if(matrixA[g][0].equals(rs.getString("FIO"))) {
            ...
            void vivod(){fr.getContentPane().removeAll();
                ...
                Connection conn = null; Statement stmt = null; ResultSet rs = null;   int Year; int Month;
                Calendar calendar = Calendar.getInstance();
                Month = calendar.get(Calendar.MONTH)+1;
                Year = calendar.get(Calendar.YEAR);
                try { Class.forName("org.h2.Driver");
            ...
            public class OK2 implements ActionListener{
                public void actionPerformed(ActionEvent event) {
                    fr.getContentPane().removeAll();
                    String[] columnName = { "ФИО", "Баллы", "Оценка"};
                    String[][] matrixB;
                    int p = 0;
                    Connection conn = null; Statement stmt = null; ResultSet rs = null;
                    ...
                    try { Class.forName("org.h2.Driver");
                        ...
                        while(rs.next()){ if(a.equals(rs.getString("DISC"))&&c.equals(rs.getString("GR"))){
                            p++;} }
                ...
                public void actionPerformed(ActionEvent event) {
                    ...
                    try { Class.forName("org.h2.Driver");
                        ...
                        {  a =  (String) table.getValueAt(i,0);
                            ...
                            stmt.executeUpdate("insert into EXAM (FIO,GR,DISC,BAL,KOL,UC,VO) VALUES ('"+a+"','"+b+"','"+c+"','"+d+"','"+e+"','"+f+"','"+g+"')");}}
                    catch  (ClassNotFoundException | SQLException ex){
                        Logger.getLogger(Test.class.getName()).log(Level.SEVERE,null,ex);}
                    finally{
                        if(conn!=null){ try{ conn.close(); stmt.close(); }
